    ...
    override fun onBindViewHolder(holder: NewsViewHolder, position: Int) {
        ...
        if (!newsItem.imageUrl.isNullOrEmpty()) {
            Glide.with(holder.itemView.context)
                ...
                .placeholder(R.drawable.baseline_people_alt_24) // Optional placeholder
                .into(holder.newsImage)
        } else {
            // If no image is available, set a placeholder image
            holder.newsImage.setImageResource(R.drawable.baseline_people_alt_24)
        }

        // Set the link action to open the article in the browser
        holder.newsLink.setOnClickListener {
