...
class XrpActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Toast.makeText(this, "XrpActivity Launched", Toast.LENGTH_SHORT).show()
        ...
        Log.d("XrpActivity", "Received data: coin_name=$coinName, current_price=$currentPrice, percentage_change=$percentageChange")
...
//                }
            ...
            try {
                ...
                if (xrpResponse.isSuccessful) {
                    ...
                    withContext(Dispatchers.Main) {
                        // Update UI for BTC
                        updateCoinUI(xrpData, "XRP")
                    }
                } else {
                    Log.e("XrpActivity", "Error fetching data for XRP: ${xrpResponse.errorBody()}")
                }
            } catch (e: Exception) {
                Log.e("XrpActivity", "Exception while fetching data: ${e.localizedMessage}")
            }
        }
    }
    ...
    private fun setupLineChart(coinData: CoinData?, coinType: String) {
        ...
        lineChart.axisLeft.axisMaximum = 10f
